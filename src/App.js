...
class App extends Component {
  ...
  tick = () => {
    this.setState(prevState => {
      ...
      return {isGameOver: true, timer: 60}
    })
  }

  onClickTab = id => {
    const newTabId = tabsList.filter(eachItem => eachItem.tabId === id)
    this.setState({tabId: newTabId[0].tabId})
  }

  onMatchImage = id => {
    const {matchImages, displayImg, timer} = this.state

    if (displayImg.id === id) {
      const remImages = matchImages.filter(eachItem => eachItem.id !== id)
      const changeImg = remImages[Math.floor(Math.random() * remImages.length)]
      if (remImages.length === 0) {
        this.gameOver()
      }

      this.setState(prevState => ({
        matchImages: remImages,
        score: prevState.score + 1,
        displayImg: changeImg,
      }))
    } else {
      this.gameOver()
    }
  ...
  render() {
    const {tabId, timer, displayImg, isGameOver, score, matchImages} =
      this.state
